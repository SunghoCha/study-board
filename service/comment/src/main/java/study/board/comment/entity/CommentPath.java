...
public class CommentPath {
    ...
    public CommentPath createChildCommentPath(String descendantsTopPath) {
        if (descendantsTopPath == null) { // path와 관련된 자손이 하나도 없는 상태
    ...
    private String increase(String childrenTopPath) {
        String lastChunk = path.substring(0, (getDepth() + 1) * DEPTH_CHUNK_SIZE);
    ...
    private String findChildrenTopPath(String descendantsTopPath) {
        return descendantsTopPath.substring(0, (getDepth() + 1) * DEPTH_CHUNK_SIZE); // descendantsTopPath : path를 갖는 자손중 가장 depth가 깊고 끝에 있는 path.
    }
