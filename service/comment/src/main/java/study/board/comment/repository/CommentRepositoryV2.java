   ...
   @Query(value = "select c from CommentV2 c where c.commentPath.path = :path")
    ...
    @Query(
            value = "select path from comment_v2 " +
                    "where article_id = :articleId and path > :pathPrefix and path like :fullPathPrefix " +
    ...
    Optional<String> findDescendantsTopPath(
            ...
            @Param("fullPathPrefix") String fullPathPrefix // fullPathPrefix = pathPrefix%
   );
    ...
    @Query(
            value = "select count(*) from (" +
                    "  select comment_v2.comment_id from comment_v2 where article_id = :articleId limit :limit " +
                    ") t",
            nativeQuery = true
    )
    Long count(
